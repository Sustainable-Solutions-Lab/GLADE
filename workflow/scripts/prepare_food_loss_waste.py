...
def apply_waste_calibration(
    result: pd.DataFrame,
    calibration_path: str,
    food_groups: list[str],
) -> pd.DataFrame:
    """Apply uniform per-group waste-retention multipliers.

    The calibration YAML maps each calibrated food group to a multiplier
    that scales ``(1 - waste_fraction)`` for every country in that group:

        new_waste = 1 - (1 - waste_after_overrides) * waste_retention_multiplier

    Groups absent from either the file or ``food_groups`` are left untouched.
    Groups listed in ``food_groups`` but missing from the file raise so a
    missing/stale calibration is loud.
    """
    with open(calibration_path) as fh:
        cal = yaml.safe_load(fh) or {}
    if not isinstance(cal, dict):
        raise ValueError(
            f"Calibration file {calibration_path} must be a YAML mapping; got "
            f"{type(cal).__name__}"
        )

    missing = [g for g in food_groups if g not in cal]
    if missing:
        raise ValueError(
            f"Food waste calibration missing entries for groups {missing} in "
            f"{calibration_path}. Regenerate with `tools/calibrate food_waste`."
        )

    for group in food_groups:
        entry = cal[group]
        if "waste_retention_multiplier" not in entry:
            raise ValueError(
                f"Calibration entry for {group} in {calibration_path} is missing "
                "'waste_retention_multiplier'"
            )
        multiplier = float(entry["waste_retention_multiplier"])
        mask = result["food_group"] == group
        if not mask.any():
            logger.warning(
                "Waste calibration for %s has no matching rows; skipping",
                group,
            )
            continue
...
def main():
    ...
    # Apply waste calibration multipliers per food group, if configured. The
    # calibration file is generated by ``compute_food_waste_calibration.py``
    # from a solved validation scenario and holds one multiplier per group:
    #   new_waste = 1 - (1 - waste_post_overrides) * waste_retention_multiplier
    # The multiplier is applied uniformly across countries within each
    # calibrated group, after curated overrides so that explicit per-row
    # overrides in the CSV always take precedence over the bulk calibration.
    waste_cal_path = snakemake.input.get("waste_calibration", None)
