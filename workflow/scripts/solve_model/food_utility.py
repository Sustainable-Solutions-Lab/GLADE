...
def add_piecewise_food_utility(n: pypsa.Network, utility_blocks_path: str) -> None:
    ...
    rows = _prepare_piecewise_utility_rows(utility_df, consume_links)
    if rows.empty:
        return

    covered_links = set(rows["name"].astype(str))
    missing_links = [
    ...
    if width_matrix.isna().any().any() or utility_matrix.isna().any().any():
        raise ValueError(
            "Incomplete utility block table after alignment; each link must define all blocks"
        )
    ...
    n.meta["food_utility_piecewise"] = {
        ...
        "blocks_per_link": len(block_ids),
        "total_width_mt_per_year": float(widths.sum().item()),
    }

    logger.info(
        "Applied piecewise utility to %d food consumption links (%d blocks each)",
        len(link_names),
        len(block_ids),
    )
